...
function upgradeNetwork () {
  docker inspect  -f '{{.Config.Volumes}}' orderer.bridgelabz.com |grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop orderer.bridgelabz.com
  docker cp -a orderer.bridgelabz.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.bridgelabz.com
  docker-compose $COMPOSE_FILES up -d --no-deps orderer.bridgelabz.com

  for PEER in peer0.importer.bridgelabz.com peer1.importer.bridgelabz.com peer0.exporter.bridgelabz.com peer1.exporter.bridgelabz.com; do
...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/importer.bridgelabz.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/exporter.bridgelabz.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/custom.bridgelabz.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA3_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/importerBank.bridgelabz.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA4_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/Insurance.bridgelabz.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# and two Peer Orgs (``Org1`` & ``exporter``) each managing and maintaining two peer nodes.
...
# (``peer0.importer.example.com`` & ``peer0.exporter.example.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile FiveOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/ImporterMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ImporterMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for ImporterMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for ExporterMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile FiveOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/ExporterMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ExporterMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for ExporterMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for CustomMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile FiveOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/CustomMSPanchors.tx -channelID $CHANNEL_NAME -asOrg CustomMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for CustomMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for ImporterBankMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile FiveOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/ImporterBankMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ImporterBankMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for ImporterbankMSP..."
    exit 1
  fi
